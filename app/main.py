...
def _call_ai(question,vehicle_name,system,restrict_to_docs=True):
    payload={"model":MODEL,"input":[
        {"role":"system","content":system},
        {"role":"user","content":f"Seçilen araç: {vehicle_name}\nSoru: {question}"}
    ]}
    if restrict_to_docs:
        payload["tools"]=[{"type":"file_search","vector_store_ids":[GLOBAL_VS],"max_num_results":8}]
...
def ask(request:Request,question:str=Form(...),vehicle_id:int=Form(...),csrf:str=Form(None)):
    ...
    if not API or not GLOBAL_VS or not MODEL:
        return templates.TemplateResponse("home.html",{"request":request,"user":u,"vehicles":db.vehicles(),
            "csrf":csrf_token(request),"remaining":remaining_ctx(),"error":"Sunucu AI yapılandırması tamamlanmamış."})

    answer,sources=_call_ai(question,vehicle_name,SYSTEM,restrict_to_docs=True)
    if answer is None:
        return templates.TemplateResponse("home.html",{"request":request,"user":u,"vehicles":db.vehicles(),
...
async def admin_document(request:Request,vehicle_id:int=Form(...),file:UploadFile=File(...),csrf:str=Form(None)):
    ...
    if API:
        ...
        if rr.ok:
            ...
            if rr.ok:
                vs=rr.json()["id"]
                db.add_doc(vehicle_id,file.filename,fid,vs,u["username"],u["id"])
    return RedirectResponse("/admin",303)
