...
templates.env.filters["safe"] = lambda x: x  # Already marked safe in Python
...
def render_markdown(text):
    """Convert markdown to HTML, preserving safety"""
    if not text:
        return ""
    html = markdown.markdown(text, extensions=['nl2br'])
    # Basic safety: only allow basic markdown tags
    return html
...
def home(request:Request):
    ...
    history = db.get_user_questions(u["id"], limit=10) if u else []
...
def login(request:Request,username:str=Form(...),password:str=Form(...)):
    ...
    db.log_activity(u["id"], "LOGIN", f"IP: {request.client.host if request.client else 'unknown'}")
...
def ask(request:Request,question:str=Form(...),vehicle_id:int=Form(...)):
    ...
    if len(question)>MAXQ:
        history = db.get_user_questions(u["id"], limit=10)
    ...
    try:
        ...
        db.log_activity(u["id"], "QUESTION_ERROR", str(exc)[:500])
        history = db.get_user_questions(u["id"], limit=10)
        return render_template("home.html", {
            "request":request,"user":u,"vehicles":db.vehicles(),
            "history":history,
    ...
    db.log_activity(u["id"], "QUESTION_ASKED", f"Vehicle: {vehicle_name}")
    ...
    return render_template("home.html", {
        ...
        "history":history,
        "remaining":max(0,DAILY-db.question_count(u["id"])),
        "answer":answer_html,"answer_raw":answer,"sources":sources,"question":question
    })
...
def admin(request:Request):
    ...
    activity_log = db.get_activity_log(limit=50)
    return render_template("admin.html",{"request":request,"user":u,"vehicles":db.vehicles(),
        "documents":db.documents(),"questions":db.recent_questions(),"activity_log":activity_log})
...
async def admin_document(request:Request,vehicle_id:int=Form(...),file:UploadFile=File(...)):
    ...
    try:
        ...
        db.add_doc(vehicle_id,file.filename,"",store_name,uploaded_by=u["id"])
