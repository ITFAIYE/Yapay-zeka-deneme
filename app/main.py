...
def register(request:Request,username:str=Form(...),password:str=Form(...)):
    ...
    if len(username)<3 or len(password)<8:
        return templates.TemplateResponse("register.html",{"request":request,"error":"Kullanıcı adı en az 3, şifre en az 8 karakter olmalı."})
    if not db.create_user(username,password):
        return templates.TemplateResponse("register.html",{"request":request,"error":"Bu kullanıcı adı zaten kullanılıyor."})
    return RedirectResponse("/login?registered=1",303)

@app.get("/login",response_class=HTMLResponse)
def login_page(request:Request):
    return templates.TemplateResponse("login.html",{"request":request,"registered":request.query_params.get("registered"),"error":None})

@app.post("/login")
def login(request:Request,username:str=Form(...),password:str=Form(...)):
    u=db.auth(username.strip(),password)
    if not u:
        return templates.TemplateResponse("login.html",{"request":request,"registered":None,"error":"Kullanıcı adı veya şifre hatalı."})
...
def ask(request:Request,question:str=Form(...),vehicle_id:int=Form(...)):
    ...
    # Use the vector store attached to the selected vehicle/document.
    # Fall back to the global vector store only when no vehicle-specific
    # vector store is available.
    vs = GLOBAL_VS
    try:
        docs = db.documents()
        vehicle_docs = [d for d in docs if d.get("vehicle_id") == vehicle_id and d.get("vector_store_id")]
        if vehicle_docs:
            vs = vehicle_docs[-1].get("vector_store_id")
    except Exception:
        pass
    ...
    if not r.ok:
        # Show the real API error instead of hiding it behind a generic message.
        try:
            api_error = r.json()
            detail = api_error.get("error", {}).get("message") or r.text
        except Exception:
            detail = r.text
        detail = str(detail)[:800]
        print("OPENAI API HATASI:", r.status_code, detail)
        err=f"AI servisi hata verdi ({r.status_code}): {detail}"
        return templates.TemplateResponse("home.html",{"request":request,"user":u,"vehicles":db.vehicles(),
