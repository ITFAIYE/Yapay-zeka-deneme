...
def init():
    with conn() as c:
        with c.cursor() as cur:
            ...
            # ---- kalıcı silme desteği: silinen kayıt referanslarını temizle/NULL'la ----
            cur.execute("ALTER TABLE documents DROP CONSTRAINT IF EXISTS documents_vehicle_id_fkey")
            cur.execute("""ALTER TABLE documents ADD CONSTRAINT documents_vehicle_id_fkey
                           FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE CASCADE""")

            cur.execute("ALTER TABLE questions DROP CONSTRAINT IF EXISTS questions_user_id_fkey")
            cur.execute("""ALTER TABLE questions ADD CONSTRAINT questions_user_id_fkey
                           FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL""")

            cur.execute("ALTER TABLE questions DROP CONSTRAINT IF EXISTS questions_vehicle_id_fkey")
            cur.execute("""ALTER TABLE questions ADD CONSTRAINT questions_vehicle_id_fkey
                           FOREIGN KEY (vehicle_id) REFERENCES vehicles(id) ON DELETE SET NULL""")

            cur.execute("ALTER TABLE sessions DROP CONSTRAINT IF EXISTS sessions_user_id_fkey")
            cur.execute("""ALTER TABLE sessions ADD CONSTRAINT sessions_user_id_fkey
                           FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE""")

            cur.execute("ALTER TABLE audit_log DROP CONSTRAINT IF EXISTS audit_log_user_id_fkey")
            cur.execute("""ALTER TABLE audit_log ADD CONSTRAINT audit_log_user_id_fkey
                           FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL""")
...
def save_question(uid, vid, q, a, s, username=None, vehicle_name=None):
    with conn() as c:
        with c.cursor() as cur:
            cur.execute(
                """INSERT INTO questions(
                       user_id, vehicle_id, question, answer, sources,
                       username_snapshot, vehicle_name_snapshot
                   ) VALUES(%s,%s,%s,%s,%s,%s,%s)""",
                (uid, vid, q, a, s, username, vehicle_name)
            )
