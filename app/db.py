...
def new_session(uid):
    ...
    c=conn()
    c.execute("INSERT INTO sessions(token,user_id) VALUES(?,?)",(t,uid))
    c.commit()
    c.close()
    return t
...
def vehicles():
    c=conn()
    rows=c.execute("SELECT * FROM vehicles WHERE active=1 ORDER BY name").fetchall()
    c.close()
    return rows
...
def add_vehicle(name):
    c=conn()
    try:
        c.execute("INSERT INTO vehicles(name) VALUES(?)",(name,))
        c.commit()
        return True
    except sqlite3.IntegrityError:
        return False
...
def add_doc(vehicle_id,filename,file_id,vs_id):
    c=conn()
    c.execute("""INSERT INTO documents(vehicle_id,filename,file_id,vector_store_id)
                 VALUES(?,?,?,?)""",(vehicle_id,filename,file_id,vs_id))
    c.commit()
    c.close()
...
def save_question(uid,vid,q,a,s):
    c=conn()
    c.execute("""INSERT INTO questions(user_id,vehicle_id,question,answer,sources)
                 VALUES(?,?,?,?,?)""",(uid,vid,q,a,s))
    c.commit()
    c.close()
